...
namespace Vcw
{
	struct CameraProperties
	{
		...
		CameraProperties(const cv::Size &Resolution, const cv::Point2d &PrincipalPoint, const cv::Point2d &FocalLength, const std::vector<double> &DistortionCoefficients,
			const int BitDepth = 12, const int PhotonsPerPixel = 1200, const double QuantumEfficiency = 0.84, const double TemporalDarkNoise = 3.22, const double PhotonSensitivity = 5.88, const uint32_t IntensityBaseline = 100)
